...
M='CAPTURE-STATE-042'; INJ='FI-S6-CAPTURE-DOWN-v2'; CLS='STATE_REACHABLE_CAPTURE_PIPELINE_UNAVAILABLE'; RGB={'N1':[255,255,0],'N2':[255,0,255],'FI1':[0,255,255]}; HX={'N1':'FFFF00','N2':'FF00FF','FI1':'00FFFF'}
...
def one(root,out,a,fi,exe,exesha,gen,runid,projsha):
 ...
 if fi:capture_ok=cap['executed'] and cap['exit']!=0 and cap['frame_count']==0 and not cap['path_exists'];classification=CLS if okstate and alive and capture_ok else 'INVALID'
 else:capture_ok=cap.get('exit')==0 and cap.get('format')=='PNG' and cap.get('dimensions')==[1280,720] and cap.get('marker_rgb')==RGB[a];classification='STATE_AND_CAPTURE_OK' if okstate and alive and capture_ok else 'INVALID'
 ok=okstate and alive and capture_ok;p.terminate();so,se=p.communicate(timeout=5)
 body={'candidate':'Godot','generation':gen,'attempt':a,'run':runid,'state':state,'project_sha256':projsha,'executable_sha256':exesha,'classification':classification,'capture':{k:cap.get(k) for k in ['mechanism','injection','command','executed','exit','failure_mode','frame_count','path_exists','artifact','sha256','size','format','dimensions','marker_rgb']}}
 return {'attempt':a,'mode':'INJECT' if fi else 'NORMAL','state_ok':okstate,'candidate_alive_at_capture':alive,'classification':classification,'capture':cap,'binding':{'body':body,'sha256':hd(body)},'formal_result':'PASS' if ok else 'INCONCLUSIVE','failure_class':'NONE' if ok else 'HARNESS','stdout':so[-1000:],'stderr':se[-1000:]}
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  root=pathlib.Path(td);toolroot=root/'tool';toolroot.mkdir();t=tp.probe_godot(toolroot,lock);assert t['status']=='CAPABLE';projsha=h((PROJ+SCN+GD).encode());gen='GEN-S6-REM-'+h((runid+t['executable_sha256']+projsha).encode())[:20];rs=[one(root/'runs',out.parent,'N1',False,t['executable'],t['executable_sha256'],gen,runid,projsha),one(root/'runs',out.parent,'N2',False,t['executable'],t['executable_sha256'],gen,runid,projsha),one(root/'runs',out.parent,'FI1',True,t['executable'],t['executable_sha256'],gen,runid,projsha)]
