...
RGB={'N1':[255,255,0],'N2':[255,0,255]}; CLS='STATE_REACHABLE_CAPTURE_PIPELINE_UNAVAILABLE'
...
def main():
 ...
 if [r.get('attempt') for r in rs]!=['N1','N2','FI1']:why+=['attempt_shape']
  ...
  with tempfile.TemporaryDirectory() as td:
   td=pathlib.Path(td);z=td/'n2.png';z.write_bytes(p1.read_bytes());q=pic(z);sub_rejected=q['sha256']!=rs[1]['capture']['sha256'] or q['marker_rgb']!=RGB['N2']
   missing_rejected=reject_missing(td/'missing.png');tamper_rejected=reject_tamper(p1,td/'tampered.png',rs[0]['capture']['sha256'])
  ...
  fi=rs[2];c=fi['capture'];b=fi['binding'];classification_ok=fi.get('classification')==CLS and b.get('body',{}).get('classification')==CLS and b.get('sha256')==hd(b.get('body',{}))
  capture_ok=fi['state_ok'] and fi['candidate_alive_at_capture'] and c['executed'] and pathlib.Path(c['command'][0]).name=='scrot' and c['exit']!=0 and c['frame_count']==0 and c['path_exists'] is False and c['failure_mode']=='REAL_SCROT_WRITE_TO_UNWRITABLE_PROC_PATH' and bool(c.get('stderr'))
