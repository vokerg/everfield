...
The reviewed predecessor world-interface bounds remain exact and are not reopened:

- `WORLD_ROLE:CONT_CONTESTED_SITE` remains `BOUNDED_SET` over exactly `WORLD_IFACE:SHARED-WORKS-JUNCTION`, `WORLD_IFACE:COMMONS-EDGE`, and `WORLD_IFACE:CULTIVATION-MARGIN`;
- `WORLD_ROLE:CONT_HISTORY_OR_EVIDENCE_SOURCE` remains `BOUND_INTERFACE` to exactly `WORLD_IFACE:HISTORY-TRACE`;
- `WORLD_ROLE:CONT_AFTERMATH_SURFACE` remains `BOUNDED_SET` over exactly `WORLD_IFACE:WATER-DEPENDENCY`, `WORLD_IFACE:SHARED-WORKS-JUNCTION`, `WORLD_IFACE:COMMONS-EDGE`, `WORLD_IFACE:CULTIVATION-MARGIN`, `WORLD_IFACE:HISTORY-TRACE`, and `WORLD_IFACE:OUTER-CONNECTION`.

Concrete selection within either bounded set is downstream-owned. No member is selected here, and widening either reviewed target set requires a later reviewed binding change.
