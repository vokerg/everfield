...
|---|---|---|---|
| `PG-REM-CI2-M01` frozen packet did not publish executable validator/corpus | MAJOR | RESOLVED | exact validator Git blob `f872f8082592be8e2f067fdf4772034d25483c5e`; self-bound source digest `sha256:96a016c998d4b1af30f2a1803c6723cdfbad64d6ad23e9ed2b3e83f5a5e5f346`; S1-S26 execute and result digest reproduces |
| `PG-REM-CI2-M02` predecessor root not independently reconstructable | MAJOR | RESOLVED | embedded exact predecessor-evidence artifact + canonical root algorithm; artifact/root `sha256:46f6e1dfd6b56eb2d62c689e0c20de7021ff51123002655550834abd04d8107d`; S22-S26 reject root/transition/artifact substitution |
...
1. repository Git blob `f872f8082592be8e2f067fdf4772034d25483c5e` fixes the exact repository bytes;
2. the validator recomputes `sha256-source-with-digest-line-sentinel-v1` before evaluation and requires `sha256:96a016c998d4b1af30f2a1803c6723cdfbad64d6ad23e9ed2b3e83f5a5e5f346`.
...
- S21: wrong replacement-set member → `INCONCLUSIVE`.
...
The transition binds exact values for predecessor, successor, changed-work identity, reason, transition ID, predecessor artifact digest, canonical root algorithm, and root. The validator does not merely require these fields to be populated; each is compared to the frozen transition contract and the predecessor artifact itself is compared to the exact frozen predecessor digest.
...
- S22 substitutes both the declared root and claimed-observed root while predecessor bytes stay fixed → `INCONCLUSIVE`;
- S23 substitutes changed-work identity → `INCONCLUSIVE`;
- S24 substitutes transition ID → `INCONCLUSIVE`;
- S25 substitutes remediation reason → `INCONCLUSIVE`;
- S26 substitutes predecessor evidence bytes and updates producer-facing digest/root consistently → `INCONCLUSIVE`.

The emitted result object retains the predecessor evidence artifact, its digest, the transition, root algorithm, and recomputed root, making later reconstruction independent of producer prose and resistant to mutually-consistent producer substitutions.
...
- successor transition exact predecessor/successor/work/reason/transition/artifact/root binding: PASS;
